...
def update_code(args, template, idx_data, idx_code, snippet_json, base, ext):
    ...
    for i, line in enumerate(snippet_json["INST"], 1):
        ...
        path = os.path.join(args.output, f"{base}_{i:04d}{ext}")
...
def main():
    parser = argparse.ArgumentParser(
        description="This scripts load a json with snippet structure and \
                     create multiple incremental snippets")
    parser.add_argument("json",
                        help="json file that contains the snippet to be split")
    parser.add_argument("template",
                        help="template file to be complete with the snippet")
    ...
    template_base, idx_data, idx_code = \
        update_data(template_base, idx_data, idx_code, snippet_json)

    template_base, idx_data, idx_code = \
        update_code(args, template_base, idx_data, idx_code, snippet_json,
                    basename, ext)
